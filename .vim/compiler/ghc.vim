...
" edited by mntnoe: only match ^module, not ^Module etc.
...
execute 'setlocal makeprg=' . g:ghc .'\ -ilib:.\ -e\ :q\ %'
...
noremap <LocalLeader>T :call GHC_ShowType(1)<cr>
noremap <LocalLeader>t :call GHC_ShowType(0)<cr>
function! GHC_ShowType(addTypeDecl)
  ...
  if !has_key(b:ghc_types,name)
    ...
    for type in split(b:ghc_types[name],' -- ')
      echo pname "::" type
      if a:addTypeDecl
        call append( line(".")-1, pname . " :: " . substitute(type, "^ ", "", ""))
      endif
    endfor
  endif
endfunction
...
noremap <LocalLeader>si :call GHC_ShowInfo()<cr>
...
function! GHC_NameCurrent()
  ...
  let l = 1
  while l<last
    let ml = matchlist( getline(l), '\C^module\s*\([^ (]*\)')
    if ml != []
      let [_,module;x] = ml
      return [module]
    endif
...
noremap <LocalLeader>ct :call GHC_CreateTagfile()<cr>
...
noremap <LocalLeader>ie :call GHC_MkImportsExplicit()<cr>
