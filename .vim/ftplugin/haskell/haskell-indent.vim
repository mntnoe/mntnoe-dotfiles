...
function! GetHaskellIndent()
    ...
    if line =~ '{$'
        return match(line, '\C\s*where \zs\|\S') + &shiftwidth
    endif

    if line =~ '\C^\(instance\|class\).*\&.*where$'
        return &shiftwidth
    endif
    ...
    if line !~ '\C\<else\>'
        let s = match(line, '\C\<if\>.*\&.*\zs\<then\>')
        if s > 0
            return s
        endif

        let s = match(line, '\C\<if\>')
        if s > 0
            return s + g:haskell_indent_if
        endif
    endif

    let s = match(line, '\C\<do\s\+\zs[^{]\|\<where\s\+\zs\w\|\<let\s\+\zs\S\|^\s*\zs|\s')
    if s > 0
        return s
    endif

    let s = match(line, '\C\<case\>')
    if s > 0
