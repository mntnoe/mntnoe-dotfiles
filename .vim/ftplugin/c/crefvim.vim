...
" load this plugin when we come across the first .c file (ie not .cpp)
if &filetype != "c"
    finish
endif
...
if !hasmapto('<Plug>CRV_CRefVimVisual')
    vmap <silent> <unique> <LocalLeader>Cr <Plug>CRV_CRefVimVisual
endif
if !hasmapto('<Plug>CRV_CRefVimNormal')
    nmap <silent> <unique> <LocalLeader>Cr <Plug>CRV_CRefVimNormal
endif
if !hasmapto('<Plug>CRV_CRefVimAsk')
    map <silent> <unique> <LocalLeader>Cw <Plug>CRV_CRefVimAsk
endif
if !hasmapto('<Plug>CRV_CRefVimInvoke')
    map <silent> <unique> <LocalLeader>Cc <Plug>CRV_CRefVimInvoke
endif
